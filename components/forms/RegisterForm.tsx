...
type FormData = z.infer<typeof PatientFormValidation>;
...
const RegisterForm = ({ user }: { user: User }) =>  {
  ...
  const onSubmit = async (values: z.infer<typeof PatientFormValidation>) => {
    setIsLoading(true);
    console.log("Form Submitted with values:", values); // Add this line to debug
    let formData;
    if(values.identificationDocument && values.identificationDocument?.length > 0){
      const blob = new Blob([values.identificationDocument[0]],{type:values.identificationDocument[0].type})
